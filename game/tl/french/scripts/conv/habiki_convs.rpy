# TODO: Translation updated at 2024-09-12 10:43

# game/scripts/conv/habiki_convs.rpy:13
translate french habiki_01_habiki_5257c9aa:

    # sh neutral "Habiki...{w=0.5} So I can call you by your first name?"
    sh neutral "Habiki...{w=0.5} Je peux donc vous appeler par votre prénom ?"

# game/scripts/conv/habiki_convs.rpy:14
translate french habiki_01_habiki_30c7c492:

    # ha nulla "As long as you remember to add 'professor'."
    ha nulla "Tant que vous n'oubliez pas d'ajouter 'professeur'."

# game/scripts/conv/habiki_convs.rpy:15
translate french habiki_01_habiki_42843d35:

    # sh surprise "Hm...{w=0.5} Bit odd, not going to lie."
    sh surprise "Hm...{w=0.5} Un peu bizarre, je ne vais pas mentir."

# game/scripts/conv/habiki_convs.rpy:18
translate french habiki_01_habiki_eaed7462:

    # ha nulla "And why would you say that?"
    ha nulla "Et pourquoi dites-vous cela ?"

# game/scripts/conv/habiki_convs.rpy:19
translate french habiki_01_habiki_7921e9c4:

    # sh neutral "You stress the necessity of a honorific, which means you give importance to social etiquette...{w=0.5} Or at least, to your title."
    sh neutral "Vous insistez sur la nécessité d'un honorifique, ce qui signifie que vous accordez de l'importance à l'étiquette sociale...{w=0.5} Ou du moins, à votre titre."

# game/scripts/conv/habiki_convs.rpy:20
translate french habiki_01_habiki_6833b483:

    # sh frown "It's a way to put some kind of distance between you and your interlocutor, and yet you give a stranger your first name instead of your family name.{w=0.3} Kind of contradictory, don't you think?"
    sh frown "C'est une façon de mettre une certaine distance entre vous et votre interlocuteur, et pourtant vous donnez à un inconnu votre prénom au lieu de votre nom de famille.{w=0.3} C'est un peu contradictoire, vous ne trouvez pas ?"

# game/scripts/conv/habiki_convs.rpy:21
translate french habiki_01_habiki_2f7bad6a:

    # ha nulla ".{w=0.3}.{w=0.3}.{w=0.5}indeed.{w=0.3} You're well steeped in our culture, I see."
    ha nulla ".{w=0.3}.{w=0.3}.{w=0.5} En effet.{w=0.3} Vous êtes bien au fait de notre culture, à ce que je vois."

# game/scripts/conv/habiki_convs.rpy:23
translate french habiki_01_habiki_5c8290c0:

    # ha nulla "There's, though, a very logical explanation...{w=0.5} My Tabitha, here, is many things to me.{w=0.3} One of her primary functions is being my bodyguard."
    ha nulla "Il y a pourtant une explication très logique...{w=0.5} Ma Tabitha, ici présente, est bien des choses à mes yeux.{w=0.3} L'une de ses principales fonctions est d'être ma garde du corps."

# game/scripts/conv/habiki_convs.rpy:25
translate french habiki_01_habiki_4877d6c9:

    # ha nulla "With that in mind, is it really odd that someone needing a two meters tall robotic bodyguard would be quite judicious, when it came to handing out his last name?"
    ha nulla "En gardant cela à l'esprit, est-il vraiment étrange que quelqu'un ayant besoin d'un garde du corps robotique de deux mètres de haut soit assez judicieux lorsqu'il s'agit de donner son nom de famille ?"

# game/scripts/conv/habiki_convs.rpy:26
translate french habiki_01_habiki_3ca4a48b:

    # sh smile ".{w=0.3}.{w=0.3}.{w=0.5}fair enough.{w=0.5} But considering the century we live in, you mean to tell me I won't find that out just by looking up 'japanese professor with android' on a search engine?"
    sh smile ".{w=0.3}.{w=0.3}.{w=0.5} Très juste.{w=0.5} Mais vu le siècle dans lequel nous vivons, vous voulez me dire que je ne le saurais pas en cherchant 'professeur japonais avec androïde' sur un moteur de recherche ?"

# game/scripts/conv/habiki_convs.rpy:29
translate french habiki_01_habiki_6e1044dc:

    # ha nulla "Quite."
    ha nulla "Tout à fait."

# game/scripts/conv/habiki_convs.rpy:30
translate french habiki_01_habiki_301c3235:

    # sh surprise "Quite...?{w=0.5} You mean, I won't?"
    sh surprise "Tout à fait... ?{w=0.5} Vous voulez dire que je ne le trouverai pas ?"

# game/scripts/conv/habiki_convs.rpy:33
translate french habiki_01_habiki_4c33c1c6:

    # ha nulla "You won't."
    ha nulla "Vous ne le trouverez pas."

# game/scripts/conv/habiki_convs.rpy:34
translate french habiki_01_habiki_063ecc80:

    # sh surprise "Uhm...{w=0.5} Okay, then?"
    sh surprise "Uhm...{w=0.5} D'accord ?"

# game/scripts/conv/habiki_convs.rpy:35
translate french habiki_01_habiki_4ef138aa:

    # sh_i surprise "(Is he serious...?{w=0.5} Granted, it's not the first instance I hear of someone commanding a complete information blackout on surface Internet.)"
    sh_i surprise "(Il est sérieux... ?{w=0.5} Certes, ce n'est pas la première fois que j'entends parler de quelqu'un qui ordonne un blocage complet de la diffusion d'informations sur l'Internet.)"

# game/scripts/conv/habiki_convs.rpy:36
translate french habiki_01_habiki_f3391a6b:

    # sh_i frown "(But the kind of power and influence you need to actually {i}obtain{/i} such a thing are...)"
    sh_i frown "(Mais le genre de pouvoir et d'influence dont on a besoin pour {i}obtenir{/i} une telle chose sont...)"

# game/scripts/conv/habiki_convs.rpy:44
translate french habiki_01_tabitha_82aa3c47:

    # sh frown ".{w=0.3}.{w=0.3}.{w=0.5}Tabitha...{w=0.5} Why did you give it a human name?"
    sh frown ".{w=0.3}.{w=0.3}.{w=0.5} Tabitha...{w=0.5} Pourquoi lui avez-vous donné un nom humain ?"

# game/scripts/conv/habiki_convs.rpy:45
translate french habiki_01_tabitha_3bd866f3:

    # ha nulla "I feel a certain animosity, Shigeo-kun.{w=0.3} I assure you that Tabitha didn't mean you no harm."
    ha nulla "Je ressens une certaine animosité, Shigeo-kun.{w=0.3} Je vous assure que Tabitha ne vous voulait aucun mal."

# game/scripts/conv/habiki_convs.rpy:46
translate french habiki_01_tabitha_62d5fa88:

    # sh frown "Gave me quite the fright, that's for sure...{w=0.5} All I wanted to do was close the door to the smoking room properly."
    sh frown "J'ai eu une sacrée frayeur, c'est sûr...{w=0.5} Tout ce que je voulais, c'était fermer correctement la porte du fumoir."

# game/scripts/conv/habiki_convs.rpy:48
translate french habiki_01_tabitha_c504f797:

    # ha nulla "Not a fan of smoking either, I assume.{w=0.3} An awful vice to have, I agree."
    ha nulla "Pas un fan de la cigarette non plus, je suppose.{w=0.3} C'est un vice affreux, j'en conviens."

# game/scripts/conv/habiki_convs.rpy:50
translate french habiki_01_tabitha_8e010e7d:

    # ha nulla "But the drive to the château was quite long and tiring...{w=0.5} Hence the need to indulge.{w=0.3} But I fear that I could not persuade Tabitha to fully close the door."
    ha nulla "Mais le trajet jusqu'au château a été assez long et fatigant...{w=0.5} D'où la nécessité de se faire plaisir.{w=0.3} Mais je crains de ne pas avoir pu persuader Tabitha de fermer complètement la porte."

# game/scripts/conv/habiki_convs.rpy:54
translate french habiki_01_tabitha_8648360c:

    # sh surprise "You could not...{w=0.5} Persuade your Android?"
    sh surprise "Vous n'avez pas pu...{w=0.5} Persuader votre androïde ?"

# game/scripts/conv/habiki_convs.rpy:56
translate french habiki_01_tabitha_51c15520:

    # ha_x nulla "Oh, if I really wanted I could've ordered her to...{w=0.5} But I find that going against actions dictated by her prime directives is quite disruptive."
    ha_x nulla "Oh, si j'avais vraiment voulu, j'aurais pu lui ordonner de le faire...{w=0.5} Mais je trouve qu'aller à l'encontre des actions dictées par ses directives principales est assez perturbant."

# game/scripts/conv/habiki_convs.rpy:57
translate french habiki_01_tabitha_8d09aa11:

    # sh frown "Disruptive to...{w=0.5} What, exactly?"
    sh frown "Perturbant pour...{w=0.5} Quoi, exactement ?"

# game/scripts/conv/habiki_convs.rpy:60
translate french habiki_01_tabitha_642b3c30:

    # ha nulla "That, Shigeo-kun...{w=0.5} Is private."
    ha nulla "Cela, Shigeo-kun...{w=0.5} Est privé."

# game/scripts/conv/habiki_convs.rpy:61
translate french habiki_01_tabitha_1f5c4ae2:

    # sh frown ".{w=0.3}.{w=0.3}.{w=0.5}fine.{w=0.3} As long as you can actually control it."
    sh frown ".{w=0.3}.{w=0.3}.{w=0.5} Très bien.{w=0.3} Tant que vous pouvez la maîtriser."

# game/scripts/conv/habiki_convs.rpy:63
translate french habiki_01_tabitha_644a2a0a:

    # ha nulla "Hahaha, why wouldn't I?!"
    ha nulla "Hahaha, pourquoi je ne le pourrais pas ?!"

# game/scripts/conv/habiki_convs.rpy:64
translate french habiki_01_tabitha_b87a74bb:

    # sh_i frown "(I don't know, you tell me, professor 'I couldn't persuade'.)"
    sh_i frown "(Je ne sais pas, dites-le moi, professeur 'je n'ai pas pu persuader'.)"

# game/scripts/conv/habiki_convs.rpy:75
translate french habiki_01_wedding_cfe5c934:

    # sh surprise "Don't think I've seen you at the ceremony, nor the reception so far.{w=0.3} I would remember an elderly academic shadowed by a two meters tall android."
    sh surprise "Je ne pense pas vous avoir vu à la cérémonie, ni à la réception jusqu'à présent.{w=0.3} Je me souviendrais certainement d'un vieil universitaire suivi d'un androïde de deux mètres de haut."

# game/scripts/conv/habiki_convs.rpy:77
translate french habiki_01_wedding_2d88bc30:

    # ha nulla "Oh, you would be surprised...{w=0.5} But indeed, we've but recently arrived."
    ha nulla "Oh, vous seriez surpris...{w=0.5} Mais en effet, nous venons d'arriver."

# game/scripts/conv/habiki_convs.rpy:79
translate french habiki_01_wedding_a9cdcfc9:

    # sh surprise "If you've only recently arrived, that's why I haven't seen you at the ceremony or the reception.{w=0.3} I would remember an elderly academic shadowed by a two meters tall android."
    sh surprise "Si vous êtes arrivés récemment, voilà pourquoi je ne vous ai pas vus à la cérémonie ou à la réception.{w=0.3} Je me souviendrais d'un vieil universitaire suivi par un androïde de deux mètres de haut."

# game/scripts/conv/habiki_convs.rpy:81
translate french habiki_01_wedding_5be72c1b:

    # ha nulla "Oh, you would be surprised...{w=0.5} But indeed, we haven't had the opportunity to greet the bride and groom yet, truth tell."
    ha nulla "Oh, vous seriez surpris...{w=0.5} Mais en effet, nous n'avons pas encore eu l'occasion de saluer les mariés, à vrai dire."

# game/scripts/conv/habiki_convs.rpy:83
translate french habiki_01_wedding_f1bb952b:

    # ha nulla "I was supposed to arrive earlier but bad weather delayed my flight...{w=0.5} I hope young Francesco didn't take offense with it."
    ha nulla "Je devais arriver plus tôt, mais le mauvais temps a retardé mon vol...{w=0.5} J'espère que le jeune Francesco ne s'en est pas offusqué."

# game/scripts/conv/habiki_convs.rpy:84
translate french habiki_01_wedding_eda5f931:

    # sh surprise "Francesco...{w=0.5} So you're with the groom's party."
    sh surprise "Francesco...{w=0.5} Alors vous êtes avec le cortège du marié."

# game/scripts/conv/habiki_convs.rpy:86
translate french habiki_01_wedding_0bb00443:

    # ha nulla "Indeed.{w=0.3} And so do you, I believe."
    ha nulla "En effet.{w=0.3} Et vous aussi, je suppose."

# game/scripts/conv/habiki_convs.rpy:88
translate french habiki_01_wedding_d1529ed2:

    # sh neutral "Well, yes, but...{w=0.5} How can you tell?"
    sh neutral "Eh bien, oui, mais...{w=0.5} Comment le savez-vous ?"

# game/scripts/conv/habiki_convs.rpy:90
translate french habiki_01_wedding_40b587e3:

    # ha nulla "Are you kidding me?{w=0.3} Delphine's family and male acquaintances wouldn't be caught dead in a suit that cheap."
    ha nulla "Vous plaisantez ?{w=0.3} La famille et les fréquentations mâles de Delphine ne se laisseraient pas prendre dans un costume aussi bon marché."

# game/scripts/conv/habiki_convs.rpy:91
translate french habiki_01_wedding_a564ade6:

    # sh surprise sweat "Ch-!{w=0.3} This suit cost me 800 Euros!{w=0.3} I had to save for it!"
    sh surprise sweat "Bo- !{w=0.3} Ce costume m'a coûté 800 euros !{w=0.3} J'ai dû faire des économies pour l'acheter !"

# game/scripts/conv/habiki_convs.rpy:93
translate french habiki_01_wedding_d16920ea:

    # ha nulla "And the ones I've seen them wear at similar occasions range in the tens of thousands.{w=0.3} Like many things in this world, Shigeo-kun, cheapness is a...{w=0.5} Relative concept."
    ha nulla "Et ceux que je les ai vus porter à des occasions similaires se comptent en dizaines de milliers.{w=0.3} Comme bien des choses en ce monde, Shigeo-kun, le rabais est un...{w=0.5} Un principe relatif."

# game/scripts/conv/habiki_convs.rpy:95
translate french habiki_01_wedding_5bac250b:

    # sh frown -sweat "How do you know them, anyway?"
    sh frown -sweat "Comment les connaissez-vous, d'ailleurs ?"

# game/scripts/conv/habiki_convs.rpy:96
translate french habiki_01_wedding_75ec83b3:

    # ha nulla "Young Francesco was my pupil.{w=0.3} One of the finest students I've ever had the honour of teaching...{w=0.5} And you?"
    ha nulla "Le jeune Francesco était mon élève.{w=0.3} L'un des meilleurs élèves que j'ai eu l'honneur d'enseigner...{w=0.5} Et vous ?"

# game/scripts/conv/habiki_convs.rpy:97
translate french habiki_01_wedding_5211b2b5:

    # sh neutral ".{w=0.3}.{w=0.3}.{w=0.5}Francesco is my childhood friend.{w=0.3} We grew up together."
    sh neutral ".{w=0.3}.{w=0.3}.{w=0.5} Francesco est mon ami d'enfance.{w=0.3} Nous avons grandi ensemble."

# game/scripts/conv/habiki_convs.rpy:101
translate french habiki_01_wedding_d47daa85:

    # ha nulla "Indeed?{w=0.5} Hmm."
    ha nulla "Vraiment ?{w=0.5} Hmm."

# game/scripts/conv/habiki_convs.rpy:102
translate french habiki_01_wedding_c66eac4b:

    # sh surprise "Huh?{w=0.5} Something wrong?"
    sh surprise "Huh ?{w=0.5} Quelque chose ne va pas ?"

# game/scripts/conv/habiki_convs.rpy:104
translate french habiki_01_wedding_cc7424bb:

    # ha nulla ".{w=0.3}.{w=0.3}.{w=0.5}not exactly wrong, just...{w=1.0} Nevermind."
    ha nulla ".{w=0.3}.{w=0.3}.{w=0.5} Ce n'est pas exactement un problème, c'est juste...{w=1.0} Peu importe."

# game/scripts/conv/habiki_convs.rpy:106
translate french habiki_01_wedding_227a3333:

    # ha nulla "Nothing of importance.{w=0.3} You're just the first person I meet from Francesco's life from before college."
    ha nulla "Rien de bien important.{w=0.3} Vous êtes juste la première personne que je rencontre de la vie de Francesco qui date d'avant l'université."

# game/scripts/conv/habiki_convs.rpy:107
translate french habiki_01_wedding_d7943f12:

    # sh surprise "Oh...{w=0.5} Yeah, to be honest, his invite was a surprise.{w=0.3} I didn't even know he had gotten married."
    sh surprise "Oh...{w=0.5} Oui, pour être honnête, son invitation a été une surprise.{w=0.3} Je ne savais même pas qu'il s'était marié."

# game/scripts/conv/habiki_convs.rpy:108
translate french habiki_01_wedding_acc12549:

    # ha nulla "I see...{w=0.5} Well, no time like the present to reconnect, don't you think?"
    ha nulla "Je vois...{w=0.5} Eh bien, il n'y a pas de meilleur moment que le présent pour renouer contact, ne pensez-vous pas ?"

# game/scripts/conv/habiki_convs.rpy:111
translate french habiki_01_wedding_d0a5ef59:

    # ha nulla "Granted, you'll have to fight tooth and nail for some one-on-one time with him, tonight...{w=0.5} Don't know if you'll have it in you."
    ha nulla "C'est sûr, vous devrez vous battre bec et ongles pour passer du temps en privé avec lui, ce soir...{w=0.5} Je ne sais pas si vous aurez ça en vous."

# game/scripts/conv/habiki_convs.rpy:112
translate french habiki_01_wedding_eddc5251:

    # sh sad "Yeah, I noticed a tendency on the bride's family's behalf to monopolize the happy couple."
    sh sad "Oui, j'ai remarqué que la famille de la mariée semblait avoir tendance à monopoliser le jeune couple."

# game/scripts/conv/habiki_convs.rpy:114
translate french habiki_01_wedding_21192eb3:

    # ha nulla "As foreseen.{w=0.3} No worries, though, Shigeo-kun.{w=0.3} I have ways to break the status quo."
    ha nulla "Comme on pouvait s'y attendre.{w=0.3} Mais ne vous inquiétez pas, Shigeo-kun.{w=0.3} J'ai des moyens de briser le statu quo."

# game/scripts/conv/habiki_convs.rpy:115
translate french habiki_01_wedding_8a9124d7:

    # sh smile "You do?{w=0.3} Please do, then."
    sh smile "Vraiment ?{w=0.3} Je vous en prie, faites-le."

# TODO: Translation updated at 2024-09-13 20:39

# game/scripts/conv/habiki_convs.rpy:85
translate french habiki_01_habiki_988796ab:

    # sh frown "It's a way to put some kind of distance between you and your interlocutor, and yet you give a stranger your first name instead of your family name.{w=0.3} Kind of contradictory, wouldn't you say?"
    sh frown "C'est une façon de mettre une certaine distance entre vous et votre interlocuteur, et pourtant vous donnez à un inconnu votre prénom au lieu de votre nom de famille.{w=0.3} C'est un peu contradictoire, vous ne trouvez pas ?"

# game/scripts/conv/habiki_convs.rpy:159
translate french habiki_01_wedding_d3e84bc7:

    # ha nulla "Are you kidding me?{w=0.3} Delphine's family and <coterie> wouldn't be caught dead in an outfit that cheap."
    ha nulla "Vous plaisantez ?{w=0.3} La famille et la <clique> de Delphine ne se laisseraient pas prendre dans un costume aussi bon marché."

# TODO: Translation updated at 2024-09-14 10:22

# game/scripts/conv/habiki_convs.rpy:91
translate french habiki_01_habiki_2123ee23:

    # sh smile ".{w=0.3}.{w=0.3}.{w=0.5}fair enough.{w=0.5} But considering the century we live in, you mean to tell me I won't find that out just by looking up 'japanese professor with Android' on a search engine?"
    sh smile ".{w=0.3}.{w=0.3}.{w=0.5} Très juste.{w=0.5} Mais vu le siècle dans lequel nous vivons, vous voulez me dire que je ne le saurais pas en cherchant 'professeur japonais avec androïde' sur un moteur de recherche ?"

# game/scripts/conv/habiki_convs.rpy:112
translate french habiki_01_tabitha_a958570e:

    # ha nulla "I feel a certain animosity, Shigeo-kun.{w=0.3} I assure you that Tabitha didn't mean you any harm."
    ha nulla "Je ressens une certaine animosité, Shigeo-kun.{w=0.3} Je vous assure que Tabitha ne vous voulait aucun mal."

# game/scripts/conv/habiki_convs.rpy:144
translate french habiki_01_wedding_58cc9890:

    # sh surprise "Don't think I've seen you at the ceremony, nor the reception so far.{w=0.3} I would remember an elderly academic shadowed by a two meters tall Android."
    sh surprise "Je ne pense pas vous avoir vu à la cérémonie, ni à la réception jusqu'à présent.{w=0.3} Je me souviendrais certainement d'un vieil universitaire suivi d'un androïde de deux mètres de haut."

# game/scripts/conv/habiki_convs.rpy:148
translate french habiki_01_wedding_5ef3738c:

    # sh surprise "If you've only recently arrived, that's why I haven't seen you at the ceremony or the reception.{w=0.3} I would remember an elderly academic shadowed by a two meters tall Android."
    sh surprise "Si vous êtes arrivés récemment, voilà pourquoi je ne vous ai pas vus à la cérémonie ou à la réception.{w=0.3} Je me souviendrais d'un vieil universitaire suivi par un androïde de deux mètres de haut."

# game/scripts/conv/habiki_convs.rpy:152
translate french habiki_01_wedding_b5fc8485:

    # ha nulla "I was supposed to arrive earlier but bad weather delayed my flight...{w=0.5} I hope young Francesco didn't take offense with my absence."
    ha nulla "Je devais arriver plus tôt, mais le mauvais temps a retardé mon vol...{w=0.5} J'espère que le jeune Francesco ne s'en est pas offusqué."

# game/scripts/conv/habiki_convs.rpy:177
translate french habiki_01_wedding_ffb5e25b:

    # ha nulla "Nothing of importance.{w=0.3} You're just the first person I meet from Francesco's life before college."
    ha nulla "Rien de bien important.{w=0.3} Vous êtes juste la première personne que je rencontre de la vie de Francesco qui date d'avant l'université."

# game/scripts/conv/habiki_convs.rpy:186
translate french habiki_01_wedding_ed7773ea:

    # sh smile "Really, now?{w=0.3} Please do, then."
    sh smile "Vraiment ?{w=0.3} S'il vous plaît, faites-le"

# TODO: Translation updated at 2024-09-15 17:23

# game/scripts/conv/habiki_convs.rpy:123
translate french habiki_01_tabitha_206d9528:

    # ha nulla "Oh, if I really wanted I could've ordered her to...{w=0.5} But I find that going against actions dictated by her prime directives is quite disruptive."
    ha nulla "Oh, si j'avais vraiment voulu, j'aurais pu lui ordonner de le faire...{w=0.5} Mais je trouve qu'aller à l'encontre des actions dictées par ses directives principales est assez perturbant."

# game/scripts/conv/habiki_convs.rpy:155
translate french habiki_01_wedding_41f4bcd5:

    # ha nulla "Indeed.{w=0.3} And so are you, I believe."
    ha nulla "En effet.{w=0.3} Et vous aussi, je suppose."

# TODO: Translation updated at 2024-09-21 16:11

# game/scripts/conv/habiki_convs.rpy:84
translate french habiki_01_habiki_b7cc1033:

    # sh neutral "You stress the necessity of an honorific, which means you give importance to social etiquette...{w=0.5} Or at least, to your title."
    sh neutral "Vous insistez sur la nécessité d'un honorifique, ce qui signifie que vous accordez de l'importance à l'étiquette sociale...{w=0.5} Ou du moins, à votre titre."

# game/scripts/conv/habiki_convs.rpy:91
translate french habiki_01_habiki_3d9abc8c:

    # sh smile ".{w=0.3}.{w=0.3}.{w=0.5}fair enough.{w=0.5} But considering the century we live in, you mean to tell me I won't find that out just by looking up 'Japanese professor with Android' on a search engine?"
    sh smile ".{w=0.3}.{w=0.3}.{w=0.5} Très juste.{w=0.5} Mais vu le siècle dans lequel nous vivons, vous voulez me dire que je ne le saurais pas en cherchant 'professeur japonais avec androïde' sur un moteur de recherche ?"

# game/scripts/conv/habiki_convs.rpy:167
translate french habiki_01_wedding_6e6a59e2:

    # ha nulla "Young Francesco was my pupil.{w=0.3} One of the finest students I've ever had the honor of teaching...{w=0.5} And you?"
    ha nulla "Le jeune Francesco était mon élève.{w=0.3} L'un des meilleurs élèves que j'ai eu l'honneur d'enseigner...{w=0.5} Et vous ?"

# game/scripts/conv/habiki_convs.rpy:177
translate french habiki_01_wedding_11254898:

    # ha nulla "Nothing of importance.{w=0.3} You're just the first person I met from Francesco's life before college."
    ha nulla "Rien de bien important.{w=0.3} Vous êtes juste la première personne que je rencontre de la vie de Francesco qui date d'avant l'université."

